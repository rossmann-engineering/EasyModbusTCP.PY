...
class ModbusClient(object):
    ...
    def read_discreteinputs(self, starting_address, quantity):
        ...
        """
        warnings.warn("deprecated, use 'read_discrete_inputs' instead", DeprecationWarning)
    ...
    def read_holdingregisters(self, starting_address, quantity):
        ...
        """
        warnings.warn("deprecated, use 'read_holding_registers' instead", DeprecationWarning)
    ...
    def read_inputregisters(self, starting_address, quantity):
        ...
        """
        warnings.warn("deprecated, use 'read_input_registers' instead", DeprecationWarning)
    ...
    def execute_read_order(self, starting_address, quantity=0, function_code: FunctionCode = FunctionCode.READ_COILS,
                           values=None):
        """
        ...
        if self.__ser is not None:
            if self.__ser.closed:
                raise exceptions.SerialPortNotOpenedException("serial port not opened")
    ...
    def write_single_coil(self, starting_address, value):
        ...
        return_value = self.execute_read_order(starting_address, function_code=FunctionCode.WRITE_SINGLE_COIL,
                                               values=value)
        return return_value
    ...
    def unitidentifier(self, unit_identifier):
        """
        Sets the Unit identifier in case of serial connection (Default = 1)
        """
        self.__unitIdentifier = unit_identifier
...
def convert_double_to_two_registers(double_value, register_order=RegisterOrder.lowHigh):
    """
    Convert 32 Bit Value to two 16 Bit Value to send as Modbus Registers
    double_value: Value to be converted
    ...
    my_list.append(int(double_value & 0x0000FFFF))  # Append Least Significant Word
    my_list.append(int((double_value & 0xFFFF0000) >> 16))  # Append Most Significant Word
    if register_order == RegisterOrder.highLow:
        my_list[0] = int((double_value & 0xFFFF0000) >> 16)
        my_list[1] = int(double_value & 0x0000FFFF)
    return my_list


def convert_float_to_two_registers(float_value, register_order=RegisterOrder.lowHigh):
    """
    Convert 32 Bit real Value to two 16 Bit Value to send as Modbus Registers
    float_value: Value to be converted
    return: 16 Bit Register values int[]
    """
    my_list = list()
    s = bytearray(struct.pack('<f', float_value))  # little endian
    my_list.append(s[0] | (s[1] << 8))  # Append Least Significant Word
    my_list.append(s[2] | (s[3] << 8))  # Append Most Significant Word
    if register_order == RegisterOrder.highLow:
        my_list[0] = s[2] | (s[3] << 8)
        my_list[1] = s[0] | (s[1] << 8)
    return my_list
...
def convert_registers_to_double(registers, register_order=RegisterOrder.lowHigh):
    ...
    - Used to receive 32 Bit values from Modbus (Modbus Registers are 16 Bit long)
    registers: 16 Bit Registers
    return: 32 bit value
    """
    return_value = (int(registers[0]) & 0x0000FFFF) | (int((registers[1]) << 16) & 0xFFFF0000)
    if register_order == RegisterOrder.highLow:
        return_value = (int(registers[1]) & 0x0000FFFF) | (int((registers[0]) << 16) & 0xFFFF0000)
    return return_value
